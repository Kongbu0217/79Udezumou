            ...
            <div class="col-md-3 mb-4"> <!-- カードを横4つに -->
                <div class="card h-100"> <!-- 高さ統一 -->
                    ...
                    checkboxes.forEach((checkbox, index) => {
                        ...
                        if (localStorage.getItem(`cardHighlight_${index}`) === "true") {
                        ...
                        checkbox.addEventListener("change", () => {
                            if (checkbox.checked) {
                                card.classList.add("highlight");
                                localStorage.setItem(`cardHighlight_${index}`, "true");
                            } else {
                                card.classList.remove("highlight");
                                localStorage.setItem(`cardHighlight_${index}`, "false");
                            }
                        });
                    });
...
checkboxes.forEach((checkbox, index) => {
    const card = cards[index];
    const cardId = card.getAttribute('data-card-id'); // data属性でカードIDを取得

    if (localStorage.getItem(`cardHighlight_${cardId}`) === "true") {
        checkbox.checked = true;
        card.classList.add("highlight");
    }
    ...
    checkbox.addEventListener("change", () => {
        if (checkbox.checked) {
            card.classList.add("highlight");
            localStorage.setItem(`cardHighlight_${cardId}`, "true");
        } else {
            card.classList.remove("highlight");
            localStorage.setItem(`cardHighlight_${cardId}`, "false");
        }
    });
});
                    </script>
